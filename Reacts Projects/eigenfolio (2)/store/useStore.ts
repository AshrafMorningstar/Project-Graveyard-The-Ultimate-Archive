...
const defaultWindows: Record<AppId, WindowState> = {
    ...
    weather: { id: 'weather', isOpen: false, isMinimized: false, isMaximized: false, zIndex: 0, position: getInitialPosition(17), size: { width: 350, height: 500 } },
...
export const useStore = create<OSState>((set, get) => ({
  ...
  createFile: (parentId, name, type, content) => set((state) => {
    // Deep clone to safely update nested structure
  ...
  deleteFile: (id) => set((state) => {
    const newFs = JSON.parse(JSON.stringify(state.fileSystem));
    const deleteFromFolder = (nodes: FileNode[]): boolean => {
        ...
        for (const node of nodes) {
            if (node.children && deleteFromFolder(node.children)) return true;
        }
        return false;
    };
    deleteFromFolder(newFs);
    return { fileSystem: newFs };
  }),
