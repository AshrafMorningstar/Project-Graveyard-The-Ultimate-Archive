...
import { Github, Twitter, Mail, MapPin, Award } from 'lucide-react';

const CosmicProfile: React.FC = () => {
  return (
    <div className="glass-panel p-8 rounded-3xl border border-neuro-purple/30 max-w-4xl mx-auto backdrop-blur-xl animate-float">
      <div className="flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="relative group">
          ...
          <div className="relative w-40 h-40 rounded-full overflow-hidden border-4 border-chronos-dark">
            <img src="https://picsum.photos/400/400" alt="Avatar" className="w-full h-full object-cover" />
          </div>
          <div className="absolute bottom-2 right-2 w-6 h-6 bg-green-500 rounded-full border-2 border-chronos-dark animate-pulse"></div>
        </div>
        ...
        <div className="flex-1 text-center md:text-left space-y-4">
          ...
          <div className="flex gap-4 justify-center md:justify-start pt-2">
            {[Github, Twitter, Mail].map((Icon, i) => (
              <button key={i} className="p-3 bg-chronos-dark/50 rounded-xl hover:bg-quantum-glow/20 hover:text-quantum-glow transition-all duration-300 border border-white/5 hover:scale-110">
                <Icon size={20} />
              </button>
            ))}
          </div>
