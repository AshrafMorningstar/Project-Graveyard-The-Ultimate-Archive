...
import { 
  ...
  Calendar as CalIcon, ChevronLeft, ChevronRight as ChevronRightIcon, CheckSquare, Type, Code2
} from 'lucide-react';
...
export const WebBrowser: React.FC = () => {
    ...
    useEffect(() => { setUrlInput(activeTab?.url || ""); }, [activeTabId]);

    const handleGo = (e: React.FormEvent) => {
        e.preventDefault();
        let u = urlInput;
        if (!u.startsWith('http') && !u.startsWith('about:')) u = 'https://' + u;
             ...
             <div className="flex-1 relative bg-white">
                 {activeTab ? (
                     <iframe 
                        ...
                        onLoad={() => updateBrowserTab(activeTab.id, { loading: false, title: activeTab.url.replace('https://', '').split('/')[0] })}
                     />
                 ) : (
